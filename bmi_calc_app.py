...
st.title("Indice de masa corporal")
st.text("Realizado por Rodi Pardo")

with st.expander('Mostrar el código en Python para calculo', expanded=True):
    with st.echo():
        ...
        with outside_expander_area:
            ...
            with c1:
                guage = st.empty()
            with c2:
                state.WEIGHT = st.number_input('Ingrese su peso (kg)', min_value=50.0, max_value=150.0, value=state.WEIGHT, step=0.5, on_change=_set_values_cb, key='weight')
                state.HEIGHT = st.number_input('Ingrese su altura (m)', min_value=1.0, max_value=2.5, value=state.HEIGHT, step=0.1, on_change=_set_values_cb, key='height')

        BMI = round(state.WEIGHT/(state.HEIGHT**2), 1)

        bmi_thresholds = [13, 18.5, 25, 30, 43]
        level_labels = ['Raquitico', 'Bajo peso','Normal','Sobrepeso','Obesidad', 'Vaya al medico']
        ...
        with outside_expander_area:
            ...
            st.subheader(f'IMC su nivel es {level} ({BMI})')
